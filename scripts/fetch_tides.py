...
# WICHTIG: extremes als Flag, nicht extremes=
url = f"https://www.worldtides.info/api/v3?extremes&lat={LAT}&lon={LON}&days={DAYS}&key={API_KEY}"

out_dir = Path("docs") / "data"
out_dir.mkdir(parents=True, exist_ok=True)
dst = out_dir / "tides.json"
raw_dst = out_dir / "tides_raw.json"   # Debug-Log

with urllib.request.urlopen(url, timeout=30) as r:
    raw = json.load(r)

# Debug-Rohantwort mitschreiben (hilft bei Fehleranalyse/Credits)
...
print(f"[OK] wrote {dst} with {len(extremes)} entries (raw saved to {raw_dst})")
